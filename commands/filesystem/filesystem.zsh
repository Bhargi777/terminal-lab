#!/usr/bin/env zsh
# bhargi files — filesystem inspection. No destructive operations here;
...
cmd_search() {
    ...
    local dir="${2:-.}"
    if [ -z "$pattern" ]; then
        echo "Usage: bhargi files search <pattern> [dir]" >&2
        return 1
    fi
...
cmd_info() {
    local target="$1"
    if [ -z "$target" ]; then
        echo "Usage: bhargi files info <path>" >&2
        return 1
    fi
    if [ ! -e "$target" ]; then
